...
void NetworkTaskManager::handleHttpGetSettings() {
    ...
    if (xSemaphoreTake(m_wifiMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        ...
        if (wifi.isConnected()) {
            String jsonResponse;
            if (wifi.getPumpSettings(jsonResponse)) {
                response.status = NetworkStatus::HTTP_OK;
                response.value = 200;
                strncpy(response.data, jsonResponse.c_str(), sizeof(response.data) - 1);
                Serial.println("[NetworkTask] HTTP GET settings successful");
            } else {
                response.status = NetworkStatus::HTTP_ERROR;
                snprintf(response.data, sizeof(response.data), "HTTP GET failed");
                Serial.println("[NetworkTask] HTTP GET settings failed");
            }
        } else {
            response.status = NetworkStatus::FAILED;
            snprintf(response.data, sizeof(response.data), "WiFi not connected");
            Serial.println("[NetworkTask] Cannot GET settings - WiFi not connected");
        }
...
void NetworkTaskManager::handleHttpPostSettings(const char* data) {
    ...
    if (xSemaphoreTake(m_wifiMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        ...
        if (wifi.isConnected()) {
            String payload(data);
            if (wifi.updatePumpSettings(payload)) {
                response.status = NetworkStatus::HTTP_OK;
                response.value = 200;
                ...
                response.status = NetworkStatus::HTTP_ERROR;
                snprintf(response.data, sizeof(response.data), "HTTP POST failed");
                Serial.println("[NetworkTask] HTTP POST settings failed");
            }
        } else {
            response.status = NetworkStatus::FAILED;
            snprintf(response.data, sizeof(response.data), "WiFi not connected");
            Serial.println("[NetworkTask] Cannot POST settings - WiFi not connected");
        }
...
void NetworkTaskManager::handleHttpPostDoseLog(const char* data) {
    ...
    if (xSemaphoreTake(m_wifiMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        ...
        if (wifi.isConnected()) {
            ...
            // Phase 3 Sprint 4: Add to retry queue when WiFi down
            response.status = NetworkStatus::FAILED;
            snprintf(response.data, sizeof(response.data), "WiFi not connected, queued for retry");
            Serial.println("[NetworkTask] Cannot POST dose log - WiFi not connected, adding to retry queue");
            addToRetryQueue(data);
        }
...
void NetworkTaskManager::processRetryQueue() {
    ...
    // Only retry if WiFi is connected
    if (xSemaphoreTake(m_wifiMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        WiFiManager& wifi = WiFiManager::getInstance();
        
        if (!wifi.isConnected()) {
            Serial.println("[NetworkTask] Retry queue: WiFi not connected, skipping");
            xSemaphoreGive(m_wifiMutex);
            return;
        }
        
        // Retry up to 3 entries per cycle
        int retried = 0;
        while (m_retryQueueCount > 0 && retried < 3) {
            ...
            snprintf(jsonBuffer, sizeof(jsonBuffer),
                     "{\"timestamp\":%lu,\"volume\":%.2f,\"success\":%s,\"pumpId\":\"%s\"}",
                     entry.timestamp, entry.volume, entry.success ? "true" : "false", entry.pumpId);
            ...
            if (wifi.postDoseLog(payload)) {
                ...
                if (entry.retryCount >= 3) {
                    ...
                    break;  // Don't retry more entries this cycle
                }
            }
