...
enum class NetworkCommand : uint8_t {
    NONE = 0,
    CONNECT_WIFI,       // Request WiFi connection
    DISCONNECT_WIFI,    // Request WiFi disconnection
    SYNC_TIME,          // Request NTP time synchronization
    HTTP_GET_SETTINGS,  // Request pump settings from server
    HTTP_POST_SETTINGS, // Send pump settings to server
    HTTP_POST_DOSE_LOG, // Send dose history event to server (Phase 2)
    HEALTH_CHECK,       // Request server health check
    GET_STATUS,         // Request current network status
    AUTO_DOSING_RESET   // Midnight reset for auto-dosing (Core 0 -> Core 1)
};
...
struct NetworkResponseMessage {
    ...
    char data[256];            // Optional response data (e.g., JSON payload)
};
...
private:
    ...
    /**
     * @brief Handle HTTP POST dose log command (Phase 2)
     */
